...
const DeleteFileModal = ({ isOpen, closeModal, file, refreshFiles }) => {
  const modalRef = useRef();

  const deleteFile = async (file) => {
    try {
      const response = await apiDELETE(`/v1/document/delete/${file?.id}`);
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-999">
        <div className="bg-white rounded-lg p-6 w-80 shadow-lg" ref={modalRef}>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              className="bg-[#E01313] text-white py-2 px-4 rounded-lg"
